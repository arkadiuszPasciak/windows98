...
test.skip("SaveFile", () => {
	test("when I fill the file name, choose a file type, and click save, the file is saved", async ({
		...
		const fileNameInput = component.getByTestId("ds-input-ma-save-file-name")
		await fileNameInput.fill("TestFile")

		const fileTypeSelect = component.getByTestId(
			"ds-select-ma-save-save-as-type",
		)
		await fileTypeSelect.selectOption("text/txt;charset=utf-8;")

		const saveButton = component.getByTestId("ma-save-file-save-button")
	...
	test("when I click the cancel button, the modal is closed", async ({
		...
		const cancelButton = component.getByTestId("ma-save-file-cancel-button")
		await cancelButton.click()

		const saveModal = component.getByTestId("ma-save-file-save-modal")
